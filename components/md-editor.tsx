...
export function MdEditor({ value, onChange, disabled }: MdEditorProps) {
  ...
  const spHeader = (
    <div className="flex md:hidden flex-col shrink-0 bg-background border-b border-border">
      ...
      <div className="flex h-24">
        {(['edit', 'preview'] as SpMode[]).map((mode) => (
          <button
            ...
            className={cn(
              'flex-1 text-xl font-medium transition-colors',
              spMode === mode
      ...
      {spMode === 'edit' && (
        <div
          className="flex overflow-x-auto py-2 px-2 gap-2.5 h-24"
        ...
        >
          {SP_TOOLS.map((tool, i) => (
            <button
              ...
              disabled={disabled}
              className="shrink-0 h-20 px-5 text-base font-mono font-medium rounded border border-border text-muted-foreground hover:text-foreground hover:bg-muted active:bg-muted disabled:opacity-40 transition-colors"
            >
              {tool.label}
            </button>
          ))}
